...
def cluster_payload(*, ready=1):
    ...
    for name in names:
        if name == "redis-cart":
            image = (
                "redis:7.2.4-alpine@" + ONLINE_BOUTIQUE_ENVIRONMENT.image_digests[name]
            )
        else:
            image = (
    ...
    services = [
        {"metadata": {"name": name}} for name in names if name != "loadgenerator"
    ]
    pods = [
        {
...
def test_traceability_records_every_requested_adapter_obligation():
    ...
    path = (
        ...
        / "testbeds"
        / "environments"
        / "online_boutique_traceability.yaml"
    )
...
""",
    ...
    runner = FakeRunner(
        [
            result(),
            result(stdout=ONLINE_BOUTIQUE_ENVIRONMENT.commit_sha + "\n"),
            result(),
            result(),
    ...
    assert (
        f"frontend:v0.9.0@{ONLINE_BOUTIQUE_ENVIRONMENT.image_digests['frontend']}"
        in apply_call[3]
    )
...
def test_observe_state_normalizes_roles_versions_digests_and_health(tmp_path):
    ...
    state = asyncio.run(
        OnlineBoutiqueAdapter(runner=runner, workspace=tmp_path).observe_state()
    )
...
def test_baseline_reports_missing_endpoints_without_hiding_ready_workloads(tmp_path):
    deployments, services, pods, _ = cluster_payload()
    adapter = OnlineBoutiqueAdapter(
        runner=FakeRunner(
            result(stdout=value)
            for value in (
                deployments,
    ...
    assert checks["pods_ready"].passed
...
def test_faults_are_targeted_and_reset_restores_state(tmp_path, fault_type, role, kind):
    runner = FakeRunner(
        [
            result(
                stdout=json.dumps(
                    deployment("redis-cart", "redis:7.2.4-alpine", replicas=2)
                )
            )
        ]
    )
    ...
    execution = asyncio.run(
        adapter.inject_fault(
            FaultSpecification(fault_type, WorkloadSelector(role), 0.5)
        )
    )
...
def test_apply_load_scales_pinned_upstream_generator_and_reset_restores_it(tmp_path):
    runner = FakeRunner(
        [
            result(
                stdout=json.dumps(
                    deployment("loadgenerator", "example/load:v0.9.0", replicas=1)
                )
            )
        ]
    )
    ...
    assert execution.active
    assert any(
        call[0][:3] == ("kubectl", "scale", "deployment/loadgenerator")
        and "--replicas=5" in call[0]
        for call in runner.calls
    )
    asyncio.run(adapter.reset())
    ...
    with pytest.raises(ValueError, match="digest"):
        asyncio.run(
            adapter.deploy_version(
                DeploymentSpecification(
                    WorkloadSelector("checkout"), "example/checkout:bad"
                )
            )
        )
    spec = DeploymentSpecification(
        WorkloadSelector("checkout"), "example/checkout:bad", "sha256:" + "a" * 64
    )
    asyncio.run(adapter.deploy_version(spec))
    asyncio.run(adapter.reset())
    assert any(
        "checkoutservice:v0.9.0@" in token for call in runner.calls for token in call[0]
    )
...
def test_failure_collects_redacted_diagnostics_and_cleanup_is_idempotent(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        asyncio.run(
            adapter.inject_fault(
                FaultSpecification(FaultType.HIGH_CPU, WorkloadSelector("cart"), 0.5)
            )
        )
    assert list((tmp_path / "diagnostics").rglob("adapter-state.json"))
    text = "\n".join(
        path.read_text() for path in (tmp_path / "diagnostics").rglob("*.*")
    )
    ...
    for area in ("apps", "services", "packages"):
        for path in (root / area).rglob("*"):
            if path.is_file() and path.suffix in {".py", ".go", ".ts", ".tsx"}:
                if "testbeds.adapters.online_boutique" in path.read_text(
                    encoding="utf-8"
                ):

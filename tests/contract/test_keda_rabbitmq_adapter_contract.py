...
def test_keda_rabbitmq_adapter_satisfies_environment_adapter_protocol(tmp_path):
    ...
    assert all(inspect.iscoroutinefunction(getattr(adapter, operation)) for operation in ("install", "reset", "wait_for_healthy_baseline", "apply_load", "inject_fault", "deploy_version", "observe_state", "cleanup"))


@pytest.mark.smoke
@pytest.mark.skipif(os.getenv("GUARDIAN_KEDA_RABBITMQ_SMOKE") != "1", reason="set GUARDIAN_KEDA_RABBITMQ_SMOKE=1 for a disposable cluster with KEDA")
def test_real_cluster_install_baseline_load_fault_reset_and_cleanup(tmp_path):
    async def scenario():
        ...
        try:
            ...
            assert (await adapter.apply_load(KEDA_RABBITMQ_ENVIRONMENT.smoke_load)).active
            await adapter.reset()
        finally:
